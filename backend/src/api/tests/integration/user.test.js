...
describe('Users API', async () => {
  ...
  describe('POST /v1/users', () => {
    ...
    it('should report error when email already exists', async () => {
      ...
      const { field: field_2 } = res.body.errors[0];
      const { location } = res.body.errors[0];
      const { messages } = res.body.errors[0];
      expect(field_2).to.be.equal('email');
    ...
    it('should report error when password length is less than 6', async () => {
      ...
      const { location } = res.body.errors[0];
      const { messages } = res.body.errors[0];
      expect(field_2).to.be.equal('password');
      expect(location).to.be.equal('body');
    ...
    it('should report error when logged user is not an admin', async () => {
      const res = await request(app)
        .post('/v1/users')
  ...
  describe('GET /v1/users', () => {
    ...
    it("should report error when pagination's parameters are not a number", async () => {
      ...
      const { location } = res.body.errors[0];
      const { messages } = res.body.errors[0];
      expect(field_2).to.be.equal('page');
      expect(location).to.be.equal('query');
      expect(messages).to.include('"page" must be a number');
      const res_1 = await Promise.resolve(res);
      const { field: field_3 } = res_1.body.errors[1];
      const { location: location_1 } = res_1.body.errors[1];
      const { messages: messages_1 } = res_1.body.errors[1];
      expect(field_3).to.be.equal('perPage');
      expect(location_1).to.be.equal('query');
      expect(messages_1).to.include('"perPage" must be a number');
    });

    it('should report error if logged user is not an admin', async () => {
      const res = await request(app)
        .get('/v1/users')
        .set('Authorization', `Bearer ${userAccessToken}`)
        .expect(httpStatus.FORBIDDEN);
  ...
  describe('GET /v1/users/profile', () => {
    it("should get the logged user's info", async () => {
      delete dbUsers.jonSnow.password;
